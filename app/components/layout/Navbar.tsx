            ...
            <nav className="hidden items-center gap-5 text-base font-semibold leading-6 text-neutral-600 md:flex">
              {links.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`inline-flex h-[24px] w-fit items-center gap-2 whitespace-nowrap transition-colors duration-200 hover:text-[#88c700] ${pathname === link.href ? "text-neutral-950" : ""
                    }`}
                >
                  {link.label}
                </Link>
